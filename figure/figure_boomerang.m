...
ax2 = subplot(2, 2, 2);  
p1 = load('FM_boomerang_s=0,7_k=5.mat');
norm1 = max(abs(p1.W2(:)));
contourf(x, x, p1.W2./norm1, 30, 'LineColor', 'none');
colormap(ax2,1-gray)
title('Factorization Method k=5, Ninc=36');


ax3 = subplot(2, 2, 3);  
p2 = load('FM_boomerang_s=0,7_k=10_N=36.mat');
norm2 = max(abs(p2.W2(:)));
contourf(x, x, p2.W2./norm2, 30, 'LineColor', 'none');
colormap(ax3,1-gray)
title('Factorization Method k=10, Ninc=36');


ax4 = subplot(2, 2, 4); 
p5 = load('FM_boomerang_s=0,7_k=10_N=72.mat');
norm5 = max(abs(p5.W2(:)));
contourf(x, x, p5.W2./norm5, 30, 'LineColor', 'none');
colormap(ax4,1-gray)
title('Factorization Method k=10, Ninc=72')
