...
ax2 = subplot(2, 2, 2);  
p1 = load('FM_cercle+rectangle_s=0,2.mat');
norm1 = max(abs(p1.W2(:)));
contourf(x, x, flipud(p1.W2./norm1), 30, 'LineColor', 'none');
colormap(ax2,1-gray)
title('Factorization Method s=0,2');


ax3 = subplot(2, 2, 3);  
p2 = load('FM_cercle+rectangle_s=0,7.mat');
norm2 = max(abs(p2.W2(:)));
contourf(x, x, flipud(p2.W2./norm2), 30, 'LineColor', 'none');
colormap(ax3,1-gray)
title('Factorization Method s=0,7');


ax4 = subplot(2, 2, 4); % third row right
p5 = load('FM_cercle+rectangle_s=1.mat');
norm5 = max(abs(p5.W2(:)));
contourf(x, x, flipud(p5.W2./norm5), 30, 'LineColor', 'none');
colormap(ax4,1-gray)
